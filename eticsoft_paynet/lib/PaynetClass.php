...
class PaynetClient
{
	...
	{
		$this->apikey = $apikey ? $apikey : Configuration::get('ETICSOFT_PAYNET_SECRET_KEY');
            ...
            foreach ($v->ratio as $r) 
			{
				...
				if($r->plus_installment >0) 
				{
					$return .= '<tr>
							...
							<td class="' . $k . '">' . number_format(($price*(1+$r->ratio)+($price*$r->ratio)*$r->ratio)/$r->instalment+$r->plus_installment, 2) . '</td>
